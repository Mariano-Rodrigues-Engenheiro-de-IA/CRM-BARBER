...
function downloadZip() {
  fetch(`/crm-assinaturas-extension-v6.zip?v=${Date.now()}`, { cache: "no-store" })
    .then((res) => {
    ...
    .then((blob) => {
      ...
      a.download = "crm-assinaturas-extension-v6.zip";
      a.click();
      ...
      <Card className="w-full max-w-lg">
        ...
        <CardContent className="space-y-4">
          <Button size="lg" className="w-full" onClick={downloadZip}>
            Baixar extensão v0.6.0 (.zip)
          </Button>
          <p className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
            Importante: remova a versão anterior em <code className="rounded bg-muted px-1">chrome://extensions</code> antes de instalar. A versão precisa aparecer como <strong>0.6.0</strong>.
          </p>
