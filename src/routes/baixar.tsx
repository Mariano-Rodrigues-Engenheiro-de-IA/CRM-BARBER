...
export const Route = createFileRoute("/baixar")({
  head: () => ({
    meta: [
      { title: "Baixar pacote da extensão v0.34.12 | Zetta CRM" },
      {
        name: "description",
        content:
          "Download direto do pacote .zip da extensão Zetta CRM v0.34.12, pronto para upload no Chrome Web Store Developer Dashboard.",
      },
      { property: "og:title", content: "Baixar pacote da extensão v0.34.12" },
      {
  ...
  const download = () => {
    ...
    fetch(`${FILE}?v=${VERSION}&t=${Date.now()}`)
      ...
      .then((blob) => {
        ...
        a.download = `zetta-crm-v3412.zip`;
        a.click();
