...
export const Route = createFileRoute("/baixar")({
  head: () => ({
    meta: [
      { title: "Baixar pacote da extensão v0.35.53 | CRM Zaylo" },
      {
        name: "description",
        content:
          "Download direto do pacote .zip da extensão Zetta CRM v0.35.53, pronto para upload no Chrome Web Store Developer Dashboard.",
      },
      { property: "og:title", content: "Baixar pacote da extensão v0.35.53" },
      {
...
const VERSION = "0.35.53";
const FILE = "/zetta-crm-v3553.zip";
  ...
  const download = () => {
    ...
    fetch(`${FILE}?v=${VERSION}&t=${Date.now()}`)
      ...
      .then((blob) => {
        ...
        a.download = `zetta-crm-v3553.zip`;
        a.click();
