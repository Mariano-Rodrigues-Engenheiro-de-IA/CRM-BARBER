...
export const Route = createFileRoute("/baixar")({
  head: () => ({
    meta: [
      { title: "Baixar pacote da extensão v0.38.1 | CRM Zaylo" },
      {
        name: "description",
        content:
          "Download direto do pacote .zip da extensão Zaylo CRM v0.38.1, pronto para upload no Chrome Web Store Developer Dashboard.",
      },
      { property: "og:title", content: "Baixar pacote da extensão v0.38.1" },
      {
...
const FILE = "/zaylo-crm-v0.38.1.zip";
  ...
  const download = () => {
    ...
    fetch(`${FILE}?v=${VERSION}&t=${Date.now()}`)
      ...
      .then((blob) => {
        ...
        a.download = `zaylo-crm-v0.38.1.zip`;
        a.click();
