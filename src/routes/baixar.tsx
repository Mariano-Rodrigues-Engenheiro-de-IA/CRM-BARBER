...
export const Route = createFileRoute("/baixar")({
  head: () => ({
    meta: [
      { title: "Baixar pacote da extensão v0.35.36 | CRM Zaylo" },
      {
        name: "description",
        content:
          "Download direto do pacote .zip da extensão Zetta CRM v0.35.36, pronto para upload no Chrome Web Store Developer Dashboard.",
      },
      { property: "og:title", content: "Baixar pacote da extensão v0.35.36" },
      {
...
const FILE = "/zetta-crm-v3536.zip";
  ...
  const download = () => {
    ...
    fetch(`${FILE}?v=${VERSION}&t=${Date.now()}`)
      ...
      .then((blob) => {
        ...
        a.download = `zetta-crm-v3536.zip`;
        a.click();
