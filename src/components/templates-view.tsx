    ...
    <div className="mt-1.5 space-y-1">
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="text-[11px] text-neutral-500">
          Clique para inserir uma variável na mensagem:
        </span>
...
export function TemplatesView({
  ...
  const {
    ...
    loading,
    refetch,
  } = useCachedFetch<Template[]>("templates", async () => {
  ...
  const [previewBox, setPreviewBox] = useState<{ top: number; left: number; width: number } | null>(
    null,
  );
  useEffect(() => {
    if (!showNew) {
  ...
  function updateButton(i: number, patch: Partial<TemplateButton>) {
    setButtons((prev) =>
      prev.map((b, bi) => (bi === i ? ({ ...b, ...patch } as TemplateButton) : b)),
    );
  }
  ...
  async function onPickCarouselFile(idx: number, file: File) {
    ...
    setCarouselCards((prev) =>
      prev.map((c, i) =>
        i === idx ? { ...c, file: { dataUrl, mime: file.type, filename: file.name } } : c,
      ),
    );
  }
  ...
  const varNames = Array.from(
    new Set(Array.from(bodyText.matchAll(/\{\{([a-z0-9_]+)\}\}/g)).map((m) => m[1])),
  );
  ...
  useEffect(() => {
    const missing = varNames.filter(
      (v) => bodyExamples[v] === undefined && DEFAULT_VARIABLE_EXAMPLES[v],
    );
  ...
  function startEdit(t: Template) {
    ...
    if (d.hasCarousel) {
      setErr(
        "Carrossel não dá pra editar por aqui. Cria um modelo novo e exclui o antigo, se quiser trocar.",
      );
      return;
    }
    resetForm();
  ...
  async function removeTemplate(name: string) {
    ...
    const r = await api(
      `/api/public/extension/whatsapp/templates?name=${encodeURIComponent(name)}`,
      { method: "DELETE" },
    );
  ...
  async function createTemplate() {
    ...
    if (varNames.some((v) => !bodyExamples[v]?.trim())) {
      setErr(
        "Preencha um valor de exemplo para cada variável. A Meta exige isso pra analisar o modelo.",
      );
      return;
    }
    ...
    setSaving(true);
    setErr(null);
    const headerFormatByType: Record<string, string> = {
      image: "IMAGE",
      video: "VIDEO",
      document: "DOCUMENT",
    };
    ...
    const res = editingId
      ? await api(`/api/public/extension/whatsapp/templates/${editingId}`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        })
      : await api("/api/public/extension/whatsapp/templates", {
          method: "POST",
          body: JSON.stringify(payload),
        });
    setSaving(false);
              ...
              <div className="space-y-4 rounded-xl border border-neutral-300 bg-white p-5 shadow-sm">
                ...
                <div>
                  ...
                  <input
                    className={
                      inputCls +
                      (editingId ? " cursor-not-allowed bg-neutral-100 text-neutral-500" : "")
                    }
                    value={name}
                    disabled={!!editingId}
                    onChange={(e) =>
                  ...
                  {editingId && (
                    <p className="mt-1 text-[11px] text-neutral-400">
                      O nome não pode ser alterado. Crie um modelo novo se precisar de outro nome.
                    </p>
                  )}
                </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setCategory(next);
                        if (next !== "MARKETING" && templateType === "carousel")
                          setTemplateType("text");
                      }}
                    >
                  ...
                  <>
                    <div>
                      ...
                      <label className="flex items-center gap-2 rounded-xl border border-dashed border-neutral-300 px-3 py-2 text-xs font-medium text-neutral-600 hover:border-brand">
                        ...
                        />
                        <span className="shrink-0 rounded-lg border border-neutral-300 px-2 py-1">
                          Escolher arquivo
                        </span>
             ...
             própria mídia e texto. Nada se mistura com os outros tipos. */}
                {templateType === "carousel" && (
                  <>
                    <div>
                      <label className="mb-1 block text-xs font-medium text-neutral-700">
                        Texto (aparece acima do carrossel)
                      </label>
                      <textarea
                        className={inputCls}
                        rows={3}
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-neutral-700">
                        Tipo de mídia dos cartões
                      </label>
                      <select
                        className={inputCls}
                        value={carouselFormat}
                        onChange={(e) => setCarouselFormat(e.target.value as "IMAGE" | "VIDEO")}
                      >
                        ...
                        {carouselButtons.map((b, i) => (
                          <div key={i} className="flex items-center gap-2">
                            <select
                              ...
                              onChange={(e) =>
                                setCarouselButtons((prev) =>
                                  prev.map((x, xi) =>
                                    xi === i
                                      ? { ...x, type: e.target.value as "URL" | "QUICK_REPLY" }
                                      : x,
                                  ),
                                )
                              }
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                setCarouselButtons((prev) => prev.filter((_, xi) => xi !== i))
                              }
                      ...
                      {carouselCards.map((card, i) => (
                        ...
                        >
                          ...
                          <label className="flex items-center gap-2 rounded-xl border border-dashed border-neutral-300 bg-white px-3 py-2 text-xs font-medium text-neutral-600 hover:border-brand">
                            ...
                            <span className="truncate">
                              {card.file ? card.file.filename : "Nenhum arquivo escolhido"}
                            </span>
                          </label>
                          <input
                            className={inputCls}
                            ...
                            onChange={(e) =>
                              setCarouselCards((prev) =>
                                prev.map((c2, ci) =>
                                  ci === i ? { ...c2, bodyText: e.target.value } : c2,
                                ),
                              )
                            }
                          />
                ...
                {varNames.length > 0 && (
                  <div className="space-y-2 rounded-lg border border-neutral-200 bg-neutral-50 p-3">
                    ...
                    <p className="text-[11px] text-neutral-500">
                      Preenchido automaticamente. Só serve pra Meta analisar o modelo, não afeta a
                      mensagem real que o cliente recebe.
                    </p>
                    {varNames.map((v) => (
                      <div key={v} className="flex items-center gap-2">
                        ...
                        <input
                          className={
                            inputCls + " cursor-not-allowed bg-neutral-100 text-neutral-500"
                          }
                    ...
                    return (
                      <tr key={t.id} className="align-top">
                        <td className="max-w-[220px] px-4 py-3">
                          ...
                          {d.bodyText && (
                            <p className="truncate text-xs text-neutral-400">{d.bodyText}</p>
                          )}
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${STATUS_STYLE[t.status] ?? "bg-neutral-200 text-neutral-700"}`}
                          >
                        ...
                        <td className="px-4 py-3 text-neutral-500">
                          {t.last_updated_time
                            ? new Date(t.last_updated_time).toLocaleDateString("pt-BR", {
                                day: "2-digit",
                                month: "2-digit",
                                year: "numeric",
                              })
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            {["APPROVED", "REJECTED", "PAUSED"].includes(t.status) &&
                              !d.hasCarousel && (
                                <button
                                  onClick={() => startEdit(t)}
                                  title="Editar"
                            ...
                            <button
                              onClick={async () => {
                                const ok = await confirm({
                                  ...
                                  description:
                                    "Isso remove o modelo (e todos os idiomas dele) de vez. Não dá pra desfazer.",
